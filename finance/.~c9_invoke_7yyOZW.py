...
def buy():
    ...
    if request.method == "POST":
        ...
        if stockInfo != None and len(stockInfo) != 0:
            cleanPrice = float(stockInfo["price"])
...
def GetUserStocks():
    ...
    #portfolio is a dictionary object, where keys are lists. transaction is just a dictionary
    for transaction in userTransactions:
        #print(transaction)
        symbol = transaction["symbol"]
        if symbol in portfolio:
            try:
                portfolio[symbol][2] = ((portfolio[symbol][0] * portfolio[symbol][2]) + (transaction["value"] * transaction["quantity"])) / (portfolio[symbol][0] + transaction["quantity"])
            ...
            portfolio[symbol] = [transaction["quantity"], lookup(symbol)["price"], transaction["value"]]
    return portfolio
